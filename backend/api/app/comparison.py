...
def compute_comparison_metrics() -> dict:
    """
    Read logs and compute metrics for each mutator type present.
    Source: CONTEXT.json → architecture.components[Comparison Engine]
    """
    # 1. Load data
    ...
    per_strategy = {mutator_type: {} for mutator_type in mutator_types}
    ...
    first_type = next(iter(mutator_types))
    cols = ["mutator_type"] + list(results[first_type].keys())
